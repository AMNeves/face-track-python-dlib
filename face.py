...
#arduino turret
def aimTurret(x1,y1,x2,y2):
    x = int((x2+x1)/2)
    y = int((y2+y1)/2)
    #transfor pixels to degrees 1280x720
    middleH = 360
    middleW = 640
    middleDegree = 90

    servoA = int( 180 - (x * middleDegree )/middleH)
    servoB = int( 180 - (y * middleDegree )/middleW)

    keya = 'a' + str(servoA) + 'b' + str(servoB) + 'z'
    arduinoData.write(keya.encode())
